...
public class Main {
  public static void main(String[] args) {
    ...
    Actor searchedActor = new Actor("Александр", "Демьяненко");
    if (filmography.containsKey(searchedActor)) {
      ArrayList<Movie> foundMovies = filmography.get(searchedActor);
      System.out.println("В фильмографии актёра А. Демьяненко найдены следующие фильмы: ");
      for (Movie movie : foundMovies) {
        if (ratings.containsKey(new Movie(movie.getTitle(), movie.getReleaseYear()))) {
          double rating = ratings.get(movie);
          System.out.println("Фильм " + movie.description() + " с рейтингом " + rating);
        } else {
          System.out.println(
              "Что-то пошло не так... Проверьте реализацию equals и hashCode в классе Movie.");
        }
      }
    } else {
      System.out.println(
          "Что-то пошло не так... Проверьте реализацию equals и hashCode в классе Actor.");
    }
  }
}
